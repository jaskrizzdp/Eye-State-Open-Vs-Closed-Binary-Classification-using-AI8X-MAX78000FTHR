...
class eyes(Dataset):
    ...
    label_to_folder_map = {'closed': 'closed', 'open': 'open'}

    def __init__(self, root_dir, d_type, transform=None,
                 resize_size=(128, 128), augment_data=False):
        self.root_dir = root_dir
        self.data_dir = os.path.join(root_dir, 'eyes', d_type)
    ...
    def __print_download_manual(self):
        print("******************************************")
        print("Eyes Open / Closed dataset not found.")
        print("Make sure images are in the following structure:")
        print("  'data/eyes/train/open'")
        print("  'data/eyes/train/closed'")
        print("  'data/eyes/test/open'")
        print("  'data/eyes/test/closed'")
        print("******************************************")
...
def get_eyes_dataset(data, load_train, load_test):
    """
    Load the Eyes Open vs Closed dataset.
    ...
    if load_train:
        train_dataset = eyes(root_dir=data_dir, d_type='train',
                                   transform=transform, augment_data=True)
    else:
        train_dataset = None

    if load_test:
        test_dataset = eyes(root_dir=data_dir, d_type='test', transform=transform)
    else:
        test_dataset = None
